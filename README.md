...
- Create an rtmpd and convert the rtmp stream into an HTTP-FLV stream.
    1. Create a controller with url `/service/httpflv`.
        ```typescript
        export default function (ctx: ServiceContext) {
        ...
        <script>
            ...
            // https://segmentfault.com/a/1190000018582618?sort=newest
            // https://www.jianshu.com/p/2d206a3fc298
            ...
            ctx.write(FLVTag.encode(0x12, metaData, 0)) // Script Tag
            ctx.flush()
            ...
            while (true) {
                ...
                ctx.write(FLVTag.encode(header.messageTypeId, data, c.clocks[header.chunkStreamId])) // messageTypeId: 8 = Audio, 9 = Video, 18 = Script
                ctx.flush()
            }
            c.close()
        ...
        class FLVTag {
            public static encode(dataType: number, data: number[], timestamp: number) {
        ...
        ```
    2. Preview at `http://127.0.0.1:8090/service/httpflv` in browser.
    3. Push a stream using ffmpeg.
        ```bash
        ffmpeg -re \
            ...
            rtmp://127.0.0.1:1935
        ```
    4. Click the Play button in browser.
