...
func init() {
	Builtins["setTimeout"] = func(worker Worker) interface{} {
		return worker.EventLoop().setTimeout
	}
	Builtins["setInterval"] = func(worker Worker) interface{} {
		return worker.EventLoop().setInterval
	}
	Builtins["setImmediate"] = func(worker Worker) interface{} {
		return worker.EventLoop().setImmediate
	}
	Builtins["clearTimeout"] = func(worker Worker) interface{} {
		return worker.EventLoop().clearTimeout
	}
	Builtins["clearInterval"] = func(worker Worker) interface{} {
		return worker.EventLoop().clearInterval
	}
	Builtins["clearImmediate"] = func(worker Worker) interface{} {
		return worker.EventLoop().clearImmediate
	}
}
...
func (loop *EventLoop) Run(fn func(*goja.Runtime) (goja.Value, error)) (goja.Value, error) {
	loop.setRunning()
	val, err := fn(loop.vm)
	loop.run(false)
	return val, err
